...
def run_scan(config, repository):
    ...
    try:
        if config.auth:
            ...
            if not config.auth.get("verify"):
                limitations.append(
                    "Supplied authentication has no verification assertion; account identity is unverified."
                )
        for item in config.endpoints:
            endpoint = Endpoint(**item)
            endpoints.setdefault(endpoint.key, endpoint)
        limits = dict(
            max_pages=config.max_pages, max_depth=config.max_depth, max_endpoints=config.max_endpoints
        )
        ...
        for endpoint in endpoints.values():
            if endpoint.method == "POST" and not config.allow_post:
                limitations.append(
                    "POST endpoints discovered but not probed; set allow_post explicitly to enable."
                )
                continue
            ...
            for parameter in parameters[: config.max_parameters]:
                for check in config.checks:
                    ...
                    try:
                        if check == "xss":
                            factory = (
                                partial(Browser, wait_ms=config.browser_wait_ms)
                                if config.verify_xss
                                else None
                            )
                        ...
                        if result:
                            ...
                            if (
                                verification.get("unavailable")
                                or verification.get("incomplete")
                                or verification.get("errors")
                            ):
                                errors.append(
                                    f"{check}: verification was incomplete; inspect finding evidence."
                                )
                        successful += 1
                    except Exception as exc:
                        errors.append(
                            f"{check} at {repository.redactor.url(endpoint.url)}: {type(exc).__name__}: {repository.redactor.text(str(exc))}"
                        )
        ...
        client.close()
        status = (
            ("partial" if successful else "failed") if errors else ("partial" if limitations else "completed")
        )
        repository.finish_scan(
            scan_id, status, errors=list(dict.fromkeys(errors)), limitations=list(dict.fromkeys(limitations))
        )
    return scan_id
