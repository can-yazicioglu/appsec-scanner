...
ERROR = re.compile(r"SQLITE_ERROR|sqlite3?\.OperationalError|SQL syntax.*?MySQL|Unclosed quotation mark|unterminated quoted string|PostgreSQL.*?ERROR|You have an error in your SQL syntax", re.I)


def signature(response, payload):
    # Remove exact reflection before comparison; do not normalize arbitrary page
    # variation into a match. Dynamic pages fail conservative confirmation.
    body = response.text
    for value in sorted({str(payload), html.escape(str(payload)), quote(str(payload)), quote_plus(str(payload))}, key=len, reverse=True):
        if value:
...
def check_sqli(client, endpoint, parameter):
    ...
    for true_payload, false_payload, control_payload in families:
        ...
        try:
            for repetition in range(3):
                probes = [("baseline", value), ("true", true_payload), ("false", false_payload), ("control", control_payload)]
                if repetition % 2:
                    probes.reverse()
                sample = {}
                for label, payload in probes:
                    result = endpoint.send(client, parameter, payload)
                    sample[label] = signature(result, payload)
                    if ERROR.search(result.text):
                        sample[label][0] = 500  # SQL errors are never differential proof.
                samples.append(sample)
                if repetition == 0 and not (sample["baseline"] == sample["true"] == sample["control"] and sample["false"] != sample["true"]):
                    break
            observations.append({"true_payload": true_payload, "false_payload": false_payload,
                                 "control_payload": control_payload, "samples": samples})
    ...
    if not confirmed and not error:
        return None
    return finding(
        "dast.sqli", "SQL injection" if confirmed else "Database error indicator",
        location("DAST", url=endpoint.url, method=endpoint.method, parameter=parameter,
                 input_location=endpoint.input_location), severity="high", confidence="confirmed" if confirmed else "suspected",
        description="Three repeated boolean differentials matched stable baseline and independent true control." if confirmed else "Database error induced by a quote; manual review required. Error text alone does not confirm SQL injection.",
        ...
        reproduction_steps=[f"Probe {parameter} at {endpoint.method} {endpoint.url} with the recorded bounded boolean expressions.",
        ...
        evidence={"request": {"method": endpoint.method, "url": client.redactor.url(endpoint.url),
                              "parameter": parameter, "payload": chosen["true_payload"] if chosen else error_payload},
                  ...
                  "verification": {"method": "boolean-differential" if confirmed else "error-indicator",
                                   "rounds": observations, "repetitions": 3 if confirmed else 0,
                                   "incomplete": failure, "signature_format": ["status", "sha256", "normalized_length"]}})
