...
def normalize_filters(severity: Iterable[str] | None = None,
                      owasp_category: Iterable[str] | None = None) -> dict[str, list[str] | None]:
    ...
    return {"severity": severities, "owasp_category": categories}
...
def _selection(values: Iterable[str] | None) -> list[str] | None:
    ...
    if isinstance(values, str):
        values = [values]
    selected = [v.strip() for v in values if isinstance(v, str) and v.strip()]
...
def export_scan(repository: ExportRepository, scan_id: str, *,
                ...
                owasp_category: Iterable[str] | None = None) -> dict[str, Any]:
    ...
    """
    filters = normalize_filters(severity, owasp_category)
